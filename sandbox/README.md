...
Lets work with the modified cosine 

```
select found_molecular_formula, found_inchi, found_pepmass, found_smiles, searched_smiles,searched_name, searched_pepmass, mass_diff, modified_cosine_score from
(
  select
    s.molecular_formula as found_molecular_formula,
    s.inchi as found_inchi,
    ...
    q.name as searched_name,
    s.pepmass - q.pepmass as mass_diff,
    pgms.modified_cosine(s.spectrum, q.spectrum) as modified_cosine_score
      from spectrums as s,
  (
    select spectrum, smiles, pepmass, name from query
  ) as q
) as t
where modified_cosine_score > 0.2 and mass_diff between -0.2 and 0.2
